exports.configure = function (express, app) {
    ...
    var sys_config = require('../../config/sys.config');
    var session = require('express-session');

    var db_configs = require('../../config/db.config');
    ...
    if (db_conn_config.session_use_connection) {
        var model_pack = require('../../core/model');
        var model = new model_pack();
        var connection = model.db.getConnection();
    ...
    var sess_object = {
        ...
        genid: function (req) {
            return common.uuid(req);// use UUIDs for session IDs
        }
    };
    ...
    var app_session = session(sess_object);
    app.use(app_session);
